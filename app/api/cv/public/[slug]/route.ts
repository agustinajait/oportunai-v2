...
export async function GET(_req: Request, { params }: { params: { slug: string } }) {
  ...
  if (cv?.experiencia?.length) {
    ...
    for (const exp of cv.experiencia) {
      ...
      if (exp.periodo) children.push(para([new TextRun({ text: exp.periodo, italics: true, size: 20, font: 'Calibri', color: '888899' })], 40));
      if (exp.descripcion) {
        const lines = exp.descripcion.split(/\n|•|-/).map(l => l.trim()).filter(Boolean);
        lines.length > 1 ? lines.forEach(l => children.push(bullet(l))) : children.push(bullet(exp.descripcion.trim()));
      }
  ...
  if (cv?.educacion?.length || cv?.nivel_estudios) {
    ...
    if (cv?.educacion?.length) {
      for (const edu of cv.educacion) {
        children.push(para([new TextRun({ text: edu.titulo, bold: true, size: 22, font: 'Calibri' })], 20));
        children.push(para([
      ...
      children.push(para([new TextRun({ text: cv.nivel_estudios, bold: true, size: 22, font: 'Calibri' })], 60));
    }
  }
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="cv-oportunai-${slug}.docx"`,
    },
  });
}
